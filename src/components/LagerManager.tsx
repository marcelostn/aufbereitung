...
const SK_V = 'lager_verbrauch_v3';
const SK_A = 'lager_anlagen_v3';

const uid = () => Math.random().toString(36).slice(2, 10);
const EUR = (n: number) =>
  new Intl.NumberFormat('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n) + ' €';
const TODAY = new Date().toISOString().slice(0, 10);
const EINHEITEN = ['ml', 'l', 'g', 'kg', 'Stück', 'Rolle', 'Flasche', 'Eimer'];
...
// ── Default-Daten (aus Excel autoaufbereiten.xlsx, Sheet "Basis") ─────────────
const DEFAULT_VERBRAUCH: Verbrauchsmittel[] = [
  // ── Chemie (Koch Chemie) ────────────────────────────────────────────────────
...
const DEFAULT_ANLAGEN: Anlagegut[] = [
...
export default function LagerManager() {
  ...
  useEffect(() => {
    setVerbrauch(loadLS(SK_V, DEFAULT_VERBRAUCH));
    setAnlagen(loadLS(SK_A, DEFAULT_ANLAGEN));
    setHydrated(true);
  }, []);
  useEffect(() => { if (hydrated) saveLS(SK_V, verbrauch); }, [verbrauch, hydrated]);
  useEffect(() => { if (hydrated) saveLS(SK_A, anlagen);   }, [anlagen,   hydrated]);
